# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 15:21:24 2020
...
data = pd.read_csv(r'C:\Users\adogan\OneDrive - Oklahoma A and M System\Classified\Codes2\TotalOutcome\TO_5DC5IC.csv')
...
rf = RandomForestClassifier(n_estimators=10000, criterion='entropy', max_depth=None, min_samples_split=200, min_samples_leaf=20, 
                            ...
                            ccp_alpha=0.0, max_samples=15142)
...
#plt.plot(rf_fpr, rf_tpr,color=[0.99,0.0,0.0], linestyle='-.',label='Random Forest (AUROC = %0.3f)' % rf_auc,lw=3, alpha=.8)
#plt.plot(svc_fbr, svc_tpr,color=[0.0,0.5,0.0], linestyle='--',label = 'Support Vector Classification(AUROC = %0.3f)' % svc_auc,lw=3, alpha=.8)
#plt.plot(kNN_fbr, kNN_tpr,color=[0.0,0.0,0.7], linestyle=':',label = 'kNN (AUROC = %0.3f)' % kNN_auc,lw=3, alpha=.8)
#plt.xlim([-0.05, 1.05])
#plt.ylim([-0.05, 1.05])
...
plot([0, 1], [0, 1])
plt.figure(figsize=(10 , 10))
plt.title('Total Outcome' ,  fontweight = "bold" , fontsize=20)
plt.plot([0, 1], [0, 1], linestyle='-', lw=3, color=[0.5,0.5,0.5],alpha=.8)
plt.plot(rf_fpr, rf_tpr,color=[0.99,0.0,0.0], linestyle='-.',label='Random Forest (AUROC = %0.3f)' % rf_auc,lw=3, alpha=.8)
plt.plot(svc_fbr, svc_tpr,color=[0.0,0.5,0.0], linestyle='--',label = 'Support Vector Classification(AUROC = %0.3f)' % svc_auc,lw=3, alpha=.8)
plt.plot(kNN_fbr, kNN_tpr,color=[0.0,0.0,0.7], linestyle=':',label = 'kNN (AUROC = %0.3f)' % kNN_auc,lw=3, alpha=.8)
...
xlabel('1 - Spesificity', fontsize=20, fontweight='bold')
ylabel('Sensivity', fontsize=20, fontweight='bold')
...
plt.figure(figsize=(10 , 10))
plt.title('Total Outcome' ,  fontweight = "bold" , fontsize=20)
pyplot.plot(lr_recall, lr_precision, color=[0.99,0.0,0.0], linestyle='-.',label=r'Mean RF (AUCPR = %0.3f)'% (lr_auc),
         lw=3)
pyplot.plot(lr_recallsvc, lr_precisionsvc, color=[0.0,0.5,0.0],linestyle='--', label=r'Mean SVC (AUCPR = %0.3f)'% (lr_aucsvc),
         lw=3)
pyplot.plot(lr_recallkNN, lr_precisionkNN, color=[0.0,0.0,0.7],linestyle=':', label=r'Mean kNN (AUCPR = %0.3f)'% (lr_auckNN),
         lw=3)
